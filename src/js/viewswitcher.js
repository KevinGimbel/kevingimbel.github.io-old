/**
 *  viewSwitcher
 * This file is linted with Standard. https://github.com/feross/standard
 */
var View = (function (window, document) {
  'use strict'
  var VIEW_ATTRIBUTE = 'data-view'
  var VIEW_ATTRIBUTE_NAME = 'view'
  var url = window.location.hash
  var pageTitle = document.querySelector('head title')
  var originalTitle = pageTitle.textContent
  var TITLE_DEVIDER = '&mdash;'
  var ERROR_VIEW = document.querySelector('[' + VIEW_ATTRIBUTE + '="error"]')
  var MENU
  ...
  function _setTitle (title) {
    if (options.changeTitle) {
      ...
      var titleRemaining = title.slice(1)
      var titleFull = titleFirstLetter.toUpperCase() + titleRemaining

      pageTitle.innerHTML = titleFull + ' ' + TITLE_DEVIDER + ' ' + originalTitle
    }
  }
  ...
  function _initActive (defaultView) {
    ...
    } else if (options.initShowAll) {
      console.log('Init active default')
 ...
 // @return url
  ...
  if (options.initShowAll) {
    console.log(this.options.initShowAll)
    Array.prototype.forEach.call(views, function (view) {
      view.style.display = 'none'
    })
  }
